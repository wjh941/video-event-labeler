...
let draftTimer=null;
function saveDraft(){if(current<0)return;clearTimeout(draftTimer);draftTimer=setTimeout(()=>{try{localStorage.setItem(DRAFT_STORAGE_KEY,JSON.stringify({sample_id:rows[current].sample_id,payload:buildPayload(false)}))}catch{}},250)}
function restoreDraft(){try{const draft=JSON.parse(localStorage.getItem(DRAFT_STORAGE_KEY)||"null");if(draft&&draft.sample_id===rows[current]?.sample_id&&draft.payload){dirty=true;return draft.payload}}catch{}return null}
