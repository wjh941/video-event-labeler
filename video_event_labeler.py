...
const DRAFT_STORAGE_KEY="video-labeler:event-draft";
let draftTimer=null;
function writeDraft(){if(current<0||!rows[current])return;try{localStorage.setItem(DRAFT_STORAGE_KEY,JSON.stringify({sample_id:rows[current].sample_id,payload:buildPayload(false)}))}catch{}}
function saveDraft(){if(current<0)return;clearTimeout(draftTimer);draftTimer=setTimeout(writeDraft,250)}
function restoreDraft(){try{const draft=JSON.parse(localStorage.getItem(DRAFT_STORAGE_KEY)||"null");if(draft&&draft.sample_id===rows[current]?.sample_id&&draft.payload){if(mode==="simple"){const inputs=eventList.querySelectorAll("input");if(inputs.length>=2){inputs[0].value=draft.payload.start_time||"";inputs[1].value=draft.payload.end_time||""}}else if(Array.isArray(draft.payload.events)){renderEvents({...rows[current],events:draft.payload.events})}dirty=true;return draft.payload}}catch{}return null}
...
function changeDirty(){dirty=true;saveDraft()}
...
async function load(){try{const status=await request("/api/status");mode=status.mode;$("dataset").textContent=status.ready?`${status.video_root_name} / ${status.csv_name}`:"未选择视频目录";$("events-section").style.display=status.ready?"block":"none";rows=await request("/api/videos");renderList();if(rows.length){await openRow(0);restoreDraft()}else setStatus(status.ready?"没有发现可播放视频":"请选择一个视频文件夹")}catch(error){setStatus(error.message,true)}}
...
def open_browser_for_server(server: LabelerHTTPServer) -> None:
    """Open the local UI and retain a terminal fallback when unavailable."""
    url = f"http://127.0.0.1:{server.server_port}/"
    try:
        if not webbrowser.open(url):
            print(f"无法自动打开浏览器，请手动访问: {url}")
    except Exception as error:
        print(f"无法自动打开浏览器，请手动访问 {url}: {error}")
...
def run_desktop_app(
    ...
    *,
    ...
    try:
        ...
        if open_browser:
            open_browser_for_server(server)
        root.mainloop()
...
def run_headless_app(state: AppState, port: int, *, open_browser: bool = False) -> None:
...
def main() -> None:
    ...
    try:
        ...
        run_headless_app(state, args.port, open_browser=not getattr(args, "no_browser", False))
        return
